"""
Задание
Напишите Python-скрипт, который выполняет следующие шаги в строгой последовательности. Используйте библиотеку mysql.connector и все операции должны быть реализованы через функции (как в примере ниже).
...
Создайте функцию insert_into_students(cursor, name, second_name), которая вставляет запись в таблицу students (поле group_id пока оставить NULL) и возвращает id нового студента.
Вызовите функцию, передав свои имя и фамилию (например, 'Иван', 'Петров'). Сохраните полученный student_id.

Шаг 3. Добавление книг для студента
Создайте список из двух книг (названия придумайте сами).
Используя executemany, вставьте обе книги в таблицу books, указав taken_by_student_id равным student_id из шага 2.

Шаг 4. Создание новой группы
Напишите функцию insert_into_groups(cursor, title, start_date, end_date), которая добавляет группу и возвращает её id.
Вызовите функцию, указав название группы (например, 'Python-2025'), дату начала и окончания (в формате 'YYYY-MM-DD'). Сохраните group_id.

Шаг 5. Обновление студента (привязка к группе)
Создайте функцию update_students_group(cursor, group_id, student_id), которая обновляет поле group_id у студента с заданным id.
...
Поставьте студенту по одной оценке за каждый из четырёх созданных уроков (значения оценок – на ваше усмотрение, от 1 до 5).
